...
//
// stockTotal < 0 means "unlimited" (mirrors stock_total IS NULL).
func stockBurnFn(stockTotal int, redeemed *int64) func(context.Context, *CashbackTransaction, *CashbackRedemption) error {
	var mu sync.Mutex
	return func(_ context.Context, _ *CashbackTransaction, _ *CashbackRedemption) error {
		mu.Lock()
		defer mu.Unlock()
...
func newStockRepo(stockTotal int, redeemed *int64) *mockRepo {
	return &mockRepo{
		getRewardFn: func(_ context.Context, id string) (*CashbackReward, error) {
			return &CashbackReward{ID: id, CustomerID: "cust-1", Cost: 50, Name: "Cafe"}, nil
		},
		getBalanceFn: func(_ context.Context, _, _ string) (float64, error) {
			return 100000, nil // saldo de sobra; el foco es el stock
		},
		burnCashbackTxFn: stockBurnFn(stockTotal, redeemed),
	}
}
...
// TestRequestRedemption_ConcurrentLastUnit: N clientes pelean por la ÚLTIMA unidad
// de forma concurrente; solo uno gana, el resto recibe "premio agotado". Nunca se
// sobre-canjea. Ejercita la garantía de concurrencia que en producción provee el
// UPDATE condicional atómico de Postgres.
func TestRequestRedemption_ConcurrentLastUnit(t *testing.T) {
	var redeemed int64
	stockTotal := 1 // solo queda 1 unidad
	svc := newTestService(newStockRepo(stockTotal, &redeemed), newMockCache())
